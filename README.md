# YesClubWebsite
This is for my clurb I'm in. Yes! Club represent!


I need an actual video for the first page (the video in the background is temporary).


The first few sentences in the first page needs revision (obvy)
the "testimonies" are random
The website will be prettier once I'm done with the rough draft
I need pictures of everybody's face

I am gonna get slimed pls help ): 
I'm cooked heh...